...
| [osm-nz](skills/osm-nz/SKILL.md) | Query OpenStreetMap Overpass API for nearby points of interest — restaurants, cafes, parks, shops, transport stops, museums, and more — around any NZ location. No login or API key required. Use when the task involves finding what is nearby, local amenities, attractions by category, or walking-distance POI search from coordinates. |
