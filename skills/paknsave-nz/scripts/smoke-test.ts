...
function run(args: string[]) {
  ...
  if (result.status !== 0) {
    throw new Error(`paknsave smoke failed: python3 ${cliPath} ${args.join(' ')}\n${result.stderr || result.stdout}`);
  }
...
if (!Array.isArray(stores) || stores.length < 1) {
  throw new Error('Expected at least one PAK\'nSAVE store for Papakura');
}
...
if (!Array.isArray(search.products)) {
  throw new Error('Expected PAK\'nSAVE search JSON to include products[]');
}

console.log('paknsave-nz smoke ok');
