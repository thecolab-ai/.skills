#!/usr/bin/env python3
"""PAK'nSAVE NZ CLI — public catalogue and authenticated account lookup.

Generated after sniffing paknsave.co.nz with Hermes+CloakBrowser CDP. Unofficial.
"""
...
AUTH_ALLOWED_HOSTS = frozenset(
    {"login.clubplus.co.nz", "api-prod.clubplus.co.nz", "api-prod.paknsave.co.nz", "www.paknsave.co.nz"}
)
DEFAULT_STORE_ID = os.environ.get("PAKNSAVE_STORE_ID", "a7d09522-bee2-41e4-8fe0-0b82b7f342f5")  # PAK'nSAVE Papakura
CACHE_DIR = Path(os.environ.get("XDG_CACHE_HOME", str(Path.home() / ".cache"))) / "paknsave-cli"
TOKEN_FILE = CACHE_DIR / "guest-token.json"
AUTH_FILE = Path(os.environ.get("PAKNSAVE_AUTH_FILE", str(CACHE_DIR / "auth.json"))).expanduser()
UA = os.environ.get(
    "PAKNSAVE_USER_AGENT",
    nzfetch.DEFAULT_UA,
)
...
def account_commands_enabled(env: Any = None) -> bool:
    ...
    username = source.get("PAKNSAVE_USERNAME") or source.get("PAKNSAVE_EMAIL")
    return bool(username and source.get("PAKNSAVE_PASSWORD"))
...
def die(msg: str, code: int = 1) -> None:
    print(f"paknsave: {msg}", file=sys.stderr)
...
def get_guest_token(force: bool = False) -> str:
    if not force and TOKEN_FILE.exists():
        try:
            ...
            pass
    fingerprint_user = os.environ.get("PAKNSAVE_FINGERPRINT_USER", "paknsave-cli")
...
def exchange_for_paknsave_tokens(access_token: str, refresh_token: str, device_id: str) -> dict[str, Any]:
    secure, _headers = auth_http(
        "POST",
        CLUBPLUS_API + "/user/token/secure",
        {"banner": BANNER_CODE, "source": "WEB"},
        token=access_token,
    )
    code = secure.get("secure_token") if isinstance(secure, dict) else None
    if not code:
        die("Club+ did not return a PAK'nSAVE secure token")
    ...
    fingerprint_user = os.environ.get("PAKNSAVE_FINGERPRINT_USER") or device_id
    sso_payload = {
        "key": code,
    ...
    if isinstance(sso, dict) and sso.get("statusCode") not in (None, 200):
        die(f"PAK'nSAVE SSO exchange failed (HTTP {sso.get('statusCode')})")
    edge_access = next(
    ...
    if not edge_access or claims.get("email") == "anonymous":
        die("PAK'nSAVE rejected the Club+ token exchange; no authenticated token was issued")
...
def finish_clubplus_login(result: dict[str, Any], device_id: str) -> dict[str, Any]:
    ...
    if result.get("isTFARequired"):
        ...
        code = os.environ.get("PAKNSAVE_MFA_CODE")
        if code:
            return complete_mfa(code)
        die(
            "Club+ requires MFA. Set PAKNSAVE_MFA_CODE to the 6-digit code and run `auth mfa`.",
            3,
        )
    if result.get("isEmailVerified") is False:
        die("Club+ requires email verification; complete that once in the PAK'nSAVE login page", 3)
    if not refresh:
        die("Club+ login response did not include a refresh token")
    return exchange_for_paknsave_tokens(str(access), str(refresh), device_id)


def login_account() -> dict[str, Any]:
    username = os.environ.get("PAKNSAVE_USERNAME") or os.environ.get("PAKNSAVE_EMAIL")
    password = os.environ.get("PAKNSAVE_PASSWORD")
    if not username or not password:
        die("set PAKNSAVE_USERNAME and PAKNSAVE_PASSWORD to log in", 2)
...
def complete_mfa(code: str | None = None) -> dict[str, Any]:
    ...
    code = (code or os.environ.get("PAKNSAVE_MFA_CODE") or "").strip()
    if len(code) != 6 or not code.isdigit():
        die("set PAKNSAVE_MFA_CODE to the 6-digit Club+ verification code", 2)
    try:
        result, _headers = auth_http(
            "POST",
...
def refresh_account(refresh_token: str | None = None) -> dict[str, Any]:
    ...
    refresh_token = refresh_token or os.environ.get("PAKNSAVE_REFRESH_TOKEN") or cached.get("refresh_token")
    if clubplus_refresh and not refresh_token:
        ...
        return exchange_for_paknsave_tokens(
            str(result["access_token"]),
            str(result["refresh_token"]),
            str(cached.get("device_id") or uuid.uuid4()),
        )
    if not refresh_token:
        die("no PAK'nSAVE refresh token is available; run `auth login`", 2)
    try:
        result, _headers = auth_http(
            "POST",
            BASE_API + "/user/login/refresh",
            {"sourceApplication": "WEB", "refreshToken": refresh_token, "banner": BANNER_CODE},
        )
    except AuthHTTPError as exc:
        die(f"PAK'nSAVE token refresh failed ({exc.code or 'HTTP ' + str(exc.status)}); run `auth login`", 2)
    if not isinstance(result, dict) or not result.get("access_token"):
        die("PAK'nSAVE token refresh returned an unexpected response")
    return save_auth_tokens(
        str(result["access_token"]),
...
def get_account_token(force_refresh: bool = False) -> str:
    env_token = os.environ.get("PAKNSAVE_ACCESS_TOKEN")
    ...
    if token and not force_refresh and token_is_fresh:
        return str(token)
    paknsave_refresh = os.environ.get("PAKNSAVE_REFRESH_TOKEN") or cached.get("refresh_token")
    if paknsave_refresh:
        return str(refresh_account(str(paknsave_refresh))["access_token"])
    if cached.get("clubplus_refresh_token"):
        return str(refresh_account()["access_token"])
    if (os.environ.get("PAKNSAVE_USERNAME") or os.environ.get("PAKNSAVE_EMAIL")) and os.environ.get(
        "PAKNSAVE_PASSWORD"
    ):
        return str(login_account()["access_token"])
    die("account authentication required; set PAK'nSAVE credentials and run `auth login`", 2)
...
def select_account_store(token: str, store_id: str = DEFAULT_STORE_ID) -> Any:
    ...
    try:
        ...
        die(f"PAK'nSAVE store selection failed ({exc.code or 'HTTP ' + str(exc.status)}){detail}")
    ...
    try:
        ...
        if (
            ...
            try:
                ...
                exc = retry_exc
        detail = f": {exc.message}" if exc.message else ""
        die(f"PAK'nSAVE account request failed ({exc.code or 'HTTP ' + str(exc.status)}){detail}")
...
def cmd_auth_login(args: argparse.Namespace) -> None:
    ...
    if args.json:
        ...
        print(f"PAK'nSAVE login complete; rotating tokens cached at {AUTH_FILE}")
...
def cmd_auth_mfa(args: argparse.Namespace) -> None:
    ...
    if args.json:
        ...
        print(f"PAK'nSAVE MFA login complete; rotating tokens cached at {AUTH_FILE}")
...
def cmd_auth_refresh(args: argparse.Namespace) -> None:
    ...
    if args.json:
        ...
        print(f"PAK'nSAVE access token refreshed; cache updated at {AUTH_FILE}")


def cmd_auth_status(args: argparse.Namespace) -> None:
    cached = read_auth_cache()
    env_access = bool(os.environ.get("PAKNSAVE_ACCESS_TOKEN"))
    token = os.environ.get("PAKNSAVE_ACCESS_TOKEN") or cached.get("access_token")
    ...
    status = {
        ...
        "refresh_available": bool(
            os.environ.get("PAKNSAVE_REFRESH_TOKEN")
...
def cmd_auth_logout(args: argparse.Namespace) -> None:
    ...
    if args.json:
        ...
        print("local PAK'nSAVE token cache removed" if removed else "no local PAK'nSAVE token cache existed")
...
def build_parser(include_account: bool | None = None) -> argparse.ArgumentParser:
    ...
    account_examples = """
          paknsave auth login
          paknsave orders --limit 20
          paknsave lists
          paknsave list-create "Weekly shop"
          paknsave list-add <list-id> 5201479
          paknsave cart
          paknsave cart-add 5201479 --quantity 2
    """ if include_account else """

        Optional account features:
          Set PAKNSAVE_USERNAME (or PAKNSAVE_EMAIL) and PAKNSAVE_PASSWORD
          before launching the CLI to enable them.
    """
    p = argparse.ArgumentParser(
        prog="paknsave",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description=f"Unofficial PAK'nSAVE NZ CLI for {scope}.",
        epilog=textwrap.dedent(f"""
        Defaults:
          store id: {DEFAULT_STORE_ID} (override with --store-id or PAKNSAVE_STORE_ID)

        Examples:
          paknsave stores --query auckland
          paknsave search milk --limit 10
          paknsave specials --limit 20
          paknsave product 5201479
          paknsave categories --depth 2
        {account_examples}
        """),
    )
    ...
    auth_sp = auth_sub.add_parser("login", help="log in using PAKNSAVE_USERNAME and PAKNSAVE_PASSWORD")
    auth_sp.add_argument("--json", action="store_true")
    auth_sp.set_defaults(func=cmd_auth_login)

    auth_sp = auth_sub.add_parser("mfa", help="complete login using PAKNSAVE_MFA_CODE")
    auth_sp.add_argument("--json", action="store_true")
    ...
    sp = sub.add_parser("lists", help="list personal PAK'nSAVE shopping lists")
    sp.add_argument("--json", action="store_true")
