#!/usr/bin/env python3
"""PAK'nSAVE NZ CLI — live product/store lookup via PAK'nSAVE web APIs.
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="Unofficial PAK'nSAVE NZ CLI for stores, categories, product search and prices.",
        epilog=textwrap.dedent(f"""
        Defaults:
          store id: {DEFAULT_STORE_ID} (override with --store-id or PAKNSAVE_STORE_ID)

        Examples:
          paknsave stores --query papakura
          paknsave search milk --limit 10
          paknsave specials cheese --limit 20
