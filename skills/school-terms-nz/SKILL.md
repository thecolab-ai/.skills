...
- Schools have flexibility over Term 1 opening and Term 4 closing dates. During the Term 1 opening window, `date` returns `school_term_or_break`; other variable dates are marked `school_dependent`.
- Published inter-term breaks are returned as fixed ranges. Summer holidays begin on each school's closing date, no later than the published boundary, and run for 5 or 6 weeks.
...
Errors are concise and emit structured JSON when `--json` is supplied. The CLI never writes to the source or local user files.
