...
A year is accepted only when it has exactly one ordered Term 1-4 record and exactly one ordered Term 1-3 break plus Summer holidays. Each date sentence must contain the exact number of unique, chronological dates for its published form, and every explicit year must match its section year. Duplicate headings, missing dates, conflicting years or incomplete sections are a source-schema error (exit 6), not an empty success.
