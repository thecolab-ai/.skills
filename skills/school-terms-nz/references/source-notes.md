...
The CLI therefore returns explicit `certainty` and `caveat` fields rather than inventing precision the Ministry does not publish. For `next-break` queries inside the inclusive Term 1 opening range, the top-level response envelope is unchanged, but `break.days_until` is `null` and `break.certainty` is `school_dependent`: an individual school may still be in summer holidays or may already be open. That conditional response also includes a clear individual-school-calendar caveat and `next_fixed_break` (`name`, `start`, `end`, `days_until`) for the next universally published inter-term break. Before the earliest opening date, summer holiday remains the current break; when the preceding year's published Summer holidays record exists it is `published`, otherwise the earliest available year uses the explicit `source_derived` fallback described above. After the latest opening date, the normal certain next fixed break is returned. During Term 4, `date` likewise reports `school_term_or_break` and `next-break` uses a null countdown because the Ministry publishes no exact school-specific closing date.
