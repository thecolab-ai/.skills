# Dimples NZ API notes

Unofficial, read-only access to public storefront routes on `https://www.dimples.co.nz`.
...
`available_online` and each variant's availability reflect Shopify's online storefront. They do not represent stock at a Dimples shop. The `stores` command returns the official Auckland and Christchurch store-finder page metadata but does not infer inventory.
