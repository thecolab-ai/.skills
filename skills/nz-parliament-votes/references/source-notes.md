...
Extraction is deliberately conservative. A division must have the exact printed “votes were recorded as follows” question and the supported `jps-JVResultParty` / `jps-JVParty` style. Party rows preserve printed labels and counts. A supported bare singleton label is retained as `entity_kind: unknown`; it is not classified as a person, party, surname, or identity. Counts must reconcile with the printed side total; otherwise that side's participant claims are discarded and warned. Missing abstentions remain `null`.
