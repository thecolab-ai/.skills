...
print("[PASS] contract rejects a cross-site product URL")
...
print("[PASS] fixture catalogue parser excludes pagination and retains product titles")
...
print("[PASS] fixture catalogue pagination path and store-location links are parsed")
...
print("[PASS] fixture Product JSON-LD yields price only with explicit NZD evidence")
data=live(["search","chicken","--limit","2","--json"])
if data is not None:
 assert data["source_url"] and data["retrieved_at"] and data["results"] and all("/product/" in row["url"] for row in data["results"]);print("[PASS] live search JSON metadata and product links")
...
if stores is not None:assert stores["source_url"] and isinstance(stores["results"],list);print("[PASS] live stores JSON")
