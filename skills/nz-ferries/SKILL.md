...
- `at-transport` = live Auckland Metro public transport data, including ferry `route_type=4` vehicle positions and departures for Fullers/AT Metro routes
...
- Live Auckland Transport ferry vehicle positions or AT Metro stop departures; use `at-transport`
- High-volume scraping or building a historical ferry timetable dataset
- Fullers360 AT Metro ferry real-time tracking; that is already covered by AT GTFS/GTFS-RT
...
- Upstream sources: SeaLink (`sealink.co.nz`), Interislander/KiwiRail (`interislander.co.nz`), Bluebridge/StraitNZ (`bluebridge.co.nz`), and Fullers360 (`fullers.co.nz`) for route/source context.
...
- Fullers360 Auckland Metro ferry live tracking is not implemented here because AT's GTFS route list already includes ferry `route_type=4`; use `at-transport` for live Auckland Metro departures, alerts, and vehicle positions. Fullers pages that require hCaptcha/PerfDrive from direct CLI fetches are documented as caveats.
- No API key, username, password, account cookie, browser session, booking mutation, or payment action is required.
