# Nature Baby NZ API notes

Unofficial, read-only access to public storefront routes on `https://www.naturebaby.co.nz`.
...
- Store information: `GET /pages/store-details`
...
`available_online` and each variant's availability reflect Shopify's online storefront. They do not represent stock at a Nature Baby shop. The `stores` command returns official page metadata but does not infer inventory.
